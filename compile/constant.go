...
func (p *Parser) constant2() (result TreeNode) {
	switch p.Token {
	case tok.String:
		node := TreeNode{name: "string", value: SuStr(p.Text)}
		node.value = p.string()
		return node
	case tok.Symbol:
		node := TreeNode{name: "symbol", value: SuStr(p.Text)}
		node.value = SuStr(p.Text)
		p.Next()
		return node
	case tok.Add:
		p.Next()
		fallthrough
	case tok.Number:
		node := TreeNode{name: "number", value: SuStr(p.Text)}
		node.value = p.number()
		return node
	case tok.Sub:
		p.Next()
		node := TreeNode{name: "number", value: SuStr(p.Text)}
		node.value = OpUnaryMinus(p.number())
		return node
	case tok.LParen, tok.LCurly, tok.LBracket:
		node := TreeNode{name: "object", value: SuStr(p.Text)}
		node.value = p.object()
		return node
	case tok.Hash:
		p.Next()
		node := TreeNode{name: "date", value: SuStr(p.Text)}
		switch p.Token {
		case tok.Number:
			node.value = p.date()
			return node
	...
	case tok.True:
		p.Next()
		node := TreeNode{name: "true", value: SuStr(p.Text)}
		node.value = True
		return node
	case tok.False:
		p.Next()
		node := TreeNode{name: "false", value: SuStr(p.Text)}
		node.value = False
		return node
	case tok.Function:
		node := TreeNode{name: "function", value: SuStr(p.Text)}
		node.value = p.functionValue()
		return node
	case tok.Class:
		node := TreeNode{name: "class", value: SuStr(p.Text)}
		node.value = p.class()
		return node
	default:
		if p.Token.IsIdent() {
