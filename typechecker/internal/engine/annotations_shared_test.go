...
func TestPropSharedAnnotationsNotMutated(t *testing.T) {
	if len(builtinAnnotations) == 0 {
		t.Fatal("annotations not loaded - test would be vacuous")
	}
	before := deepCopyAnnotations(builtinAnnotations)
	rapid.Check(t, func(rt *rapid.T) {
		...
		env := NewTypeEnv()
		RunPipeline(cls, env, &PassCtx{Annotations: builtinAnnotations}, nil)
	})
	if d := annotationsDiff(before, builtinAnnotations); d != "" {
		t.Fatalf("builtinAnnotations mutated by a pipeline run: %s", d)
	}
}
